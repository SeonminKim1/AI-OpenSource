...
# This class is used for parsing split-files where each row contains a path and a label
...
def get_transforms(train: bool = True, tfms_config: Config = None) -> Trans:
    ...
    tfms = [
        ...
        transforms.ResizeVideo(
            tfms_config.im_scale, 
            tfms_config.resize_keep_ratio
        ),
    ]
...
class VideoDataset:
    ...
    def __init__(
        # initialize dataset
        self,
        root: str, # Videos directory.
        seed: int = None, # random seed
        train_pct: float = 0.75, # percentage of dataset to use for training
        num_samples: int = 1, # Number of clips to sample from each video.
        sample_length: int = 8, # Number of consecutive frames to sample from a video (i.e. clip length).
        sample_step: int = 1, # Sampling step.
        temporal_jitter: bool = True, # Randomly skip frames when sampling each frames.
        temporal_jitter_step: int = 2, # temporal jitter in frames
        random_shift: bool = True, # Random temporal shift when sample a clip.
        batch_size: int = 8,
        video_ext: str = "mp4", # Video file extension.
        warning: bool = False, # On or off warning.
        train_split_file: str = None, #  Annotation file containing video filenames and labels.
        test_split_file: str = None, # Annotation file containing video filenames and labels.
        train_transforms: Trans = get_transforms(train=True), # transforms for training
        test_transforms: Trans = get_transforms(train=False), # transforms for testing
    ) -> None:
        ...
        print('==DEBUG, VideoDataset train_split_file, test_split_file', train_split_file, test_split_file)
        self.train_ds, self.test_ds = (
            self.split_with_file(
                train_split_file=train_split_file,
                test_split_file=test_split_file,
            )
            if train_split_file else self.split_by_folder(train_pct=train_pct)
        )

        # initialize dataloaders
        self.init_data_loaders()

    # (1) folder vs file -> (2) split train,test
    # (1-1) split by folder
    def split_by_folder(self, train_pct: float = 0.8) -> Tuple[Dataset, Dataset]:
        ...
        for action in dirs:
            ...
            self.video_records.extend(
                [
                    VideoRecord( [os.path.join(self.root, action, vid.split(".")[0]),label, action,] )
    ...
    # (1-2) Return self.split_train_test(train_range, test_range)
    def split_with_file(self, train_split_file: Union[Path, str], test_split_file: Union[Path, str],) -> Tuple[Dataset, Dataset]:
        ...
        train_range, test_range = indices[:train_len], indices[train_len:]

        return self.split_train_test(train_range, test_range)

    # (2) Return a training and testing dataset
    def split_train_test( self, train_range: torch.Tensor, test_range: torch.Tensor,) -> Tuple[Dataset, Dataset]:
    ...
    # (3) Data Loader -> self.train_dl, self.test_dl
    def init_data_loaders(self) -> None:
        """ Create training and validation data loaders. """
        devices = num_devices()
        print('==DEBUG, VideoDataset self.train_ds, self.test_ds', self.train_ds, self.test_ds)
