...
from action_recognition.dataset import get_transforms
from action_recognition.dataset import VideoDataset
...
class VideoLearner(object):
    ...
    @staticmethod
    def init_model(sample_length: int, base_model: str, num_classes: int = None ) -> torchvision.models.video.resnet.VideoResNet:
        """
        ...
        model = torch.hub.load(
            TORCH_R2PLUS1D, # github or local directory, # /home/neuralworks/.cache/torch/hub/moabitcoin_ig65m-pytorch_master
            model_name, # the name of a callable
            num_classes=MODELS[model_name],
            pretrained=True,
        )
        # print(type(model)) # torchvision.models.video.resnet.videoResnet
    ...
    def fit(self, lr: float, epochs: int, model_dir: str = "checkpoints", model_name: str = None, momentum: float = 0.95, weight_decay: float = 0.0001,
        mixed_prec: bool = False, use_one_cycle_policy: bool = False, warmup_pct: float = 0.3, lr_gamma: float = 0.1, lr_step_size: float = None, grad_steps: int = 2, save_model: bool = True,
    ) -> None:
        ...
        # Make trained model dir for Save 
        ...
        # Use mixed-precision if available :Pytorch 최적화 도구
        ...
        for e in range(1, self.epochs + 1):
            ...
            self.results.append(self.train_an_epoch(model, data_loaders, device, criterion, optimizer, grad_steps=grad_steps, mixed_prec=mixed_prec, topk=topk,))
            scheduler.step()

            if save_model:
                self.save(os.path.join(model_dir,"{model_name}_{epoch}.pt".format(model_name=model_name, epoch=str(e).zfill(3))))
        self.plot_precision_loss_curves()

    @staticmethod
    def train_an_epoch(model, data_loaders, device, criterion, optimizer, grad_steps: int = 1, mixed_prec: bool = False, topk: int = 5,) -> Dict[str, Any]:
        ...
        for phase in ["train", "valid"]:
            ...
            for step, (inputs, target) in enumerate(dl, start=1):
                ...
                inputs = inputs.to(device, non_blocking=True) # non_blocking = True, 비동기적으로 GPU 복사본을 사용가능
                target = target.to(device, non_blocking=True) # non_bolcking = True, 비동기적으로 GPU 복사본을 사용가능
    ...
    def plot_precision_loss_curves(
        ...
        plt.savefig('plot_precision_loss_curves.png')
